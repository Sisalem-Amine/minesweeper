...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # 3. Add a new sentence based on neighboring cells
        neighbors = set()

        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):

                # Skip out-of-bounds and the center cell
                if (i, j) == cell or i < 0 or j < 0 or i >= self.height or j >= self.width:
                    continue

                # Adjust count and skip if already known
        ...
        # Only add a sentence if it’s meaningful
        if neighbors:
            self.knowledge.append(Sentence(neighbors, count))

        # 4 & 5. Keep inferring until no new knowledge is gained
        knowledge_changed = True
        while knowledge_changed:
            knowledge_changed = False

            # A. Mark known mines and safes from sentences
            for sentence in self.knowledge:
            ...
            # B. Subset inference between sentence pairs
            new_sentences = []
